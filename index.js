...
function customObjectFromEntries(entries) {
  ...
  const processedEntries = entries.map((record) => {
    const [key, originalValue] = record;
...
function checkAndPreventExtensions(obj) {
  ...
  const ifCanAddProp = Object.isExtensible(obj);
  if (ifCanAddProp) {
...
function checkOwnership(obj1, obj2, key) {
  ...
  const keyInObj1 = obj1.hasOwnProperty(key);
  const keyInObj2 = obj2.hasOwnProperty(key);
...
function convertArrayToObj(arr) {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    if (obj.hasOwnProperty(key)) {
      console.log(`The array has a duplicate key: "${key}"!`);
    }
